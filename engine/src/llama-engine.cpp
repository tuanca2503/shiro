...
namespace
{
    ...
    public:
        ...
        EngineResult setToolCallGrammar(const std::string &json, const std::string &trigger) override
        {
            ...
            auto t1 = std::chrono::steady_clock::now();
            double total_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
            std::fprintf(stderr, "[DEBUG] LlamaEngine.setToolCallGrammar> from schema: %.70s... in  %.3f ms\n", json.c_str(), total_ms);
            // END DEBUG
        ...
        EngineResult setRoutesGrammar(const std::vector<std::string> &routes) override
        {
            ...
            auto t1 = std::chrono::steady_clock::now();
            double total_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
            std::fprintf(stderr, "[DEBUG] LlamaEngine.setRoutesGrammar> from routes size: %zu in  %.3f ms\n", routes.size(), total_ms);
            // END DEBUG
        ...
        EngineResult setBooleanGrammar() override
        {
            ...
            auto t1 = std::chrono::steady_clock::now();
            double total_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
            std::fprintf(stderr, "[DEBUG] LlamaEngine.setBooleanGrammar> in  %.3f ms\n", total_ms);
            // END DEBUG
        ...
        EngineResult setJsonGrammar(const std::string &json) override
        {
            ...
            auto t1 = std::chrono::steady_clock::now();
            double total_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
            std::fprintf(stderr, "[DEBUG] LlamaEngine.setJsonGrammar> from schema: %.70s... in  %.3f ms\n", json.c_str(), total_ms);
            // END DEBUG
            return setTurnGrammar(gbnf, {}, true);
        }
